...
export default {
    ...
    data(){
        return {
            ...
            GeneColorScale : { //基因颜色转换器
                ...
                maxValue:0,//颜色的边界
                self:this,
                run_init:function(minValue,maxValue){
                ...
                getColor:function(value){
                    if(value < this.self.curGeneRange[0]){
                        ...
                        let tempScale = d3.scaleLinear().domain([this.self.curGeneRange[0]-1, this.minValue]).range([0, 1]);
                    ...
                    //表达值不同的情况
                    let GeneScale = d3.scaleLinear().domain([this.minValue, this.maxValue]).range([0, 1]);
    ...
    methods:{
        ...
        drawHeatMap(){
            ...
            //初始化colorScale
            self.GeneColorScale.run_init(self.curGeneRange[0],Math.max(...contourData))
            ...
            svg.append("g")
                .attr("transform", `translate(${0},${0.05 * width})`)
                .selectAll()
                .data(value_list)
                .join("path")
            ...
            let maxValue = Math.max(...contourData)
            let minValue = self.curGeneRange[0]
            for(let i = 0;i < NumScaleLine;i++){
                legend.append("line")
                    .attr("x1",ScaleX + ScaleWidth)
                    .attr("y1",ScaleY + 0.5 * ScaleStrokeWidth + i * ((ScaleHeigt -  ScaleStrokeWidth)/(NumScaleLine-1)))
                    ...
                    .style("stroke-width","1");
                legend.append("text")
                    .text(((maxValue - minValue)*(1 - i*0.2) + minValue).toPrecision(2))
                    .attr("x",ScaleX + ScaleWidth + ScaleLineWidth + 1)
                    .attr("y",ScaleY + i * (ScaleHeigt/(NumScaleLine-1)) + ScaleLabelSize * 0.4)
                    .attr("font-size", ScaleLabelSize);
            }    
