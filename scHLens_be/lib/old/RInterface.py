...
def CellChat(matrix,cellName,geneName,label,organism):
    ...
    db_ligand: A list of ligand-receptor database, the ligand column
...
def multik(X,var_index,obs_index):
    '''
    X:adata.X
    var_index:adata.var.index
    obs_index:adata.obx.index
    ...
    r_script = open('./lib/function.R', encoding="utf-8").read()
    robjects.r(r_script)
    multik = robjects.globalenv['multik']
    result = multik(csc_matrix(X),var_index.to_list(),obs_index.to_list())
